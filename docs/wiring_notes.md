...
- 6-channel relay module selects which of six implements (grass cutter, fertilizing, height/lead-screw, weeder, seed gate, seed rotation) is connected to the shared aux BTS7960 at any given time.
...
- Wired directly to a single motor — unlike the aux driver, it is **not** shared through the relay module, so the scissor can run at the same time as any aux implement.
...
- Each button's `Click` event must call `BluetoothClient1.Send1Text` with the matching single-character command listed in the README's Control Scheme table.
- Momentary buttons (Height Up/Down, Scissor Expand/Retract) need both a press-send and a release-send configured (App Inventor: use the button's `TouchDown`/`TouchUp` events rather than `Click` for these, so the motor only runs while held).
- Toggle-style buttons (Seed Gate, Seed Rotation) can use a plain `Click` event — the firmware flips the state on each press.
- ON/OFF-pair buttons (Grass Cutter, Fertilizing, Weeder) each send a distinct fixed character — no toggle logic needed on the app side.

## Suggested Additions
- Limit switches at both ends of the lead screw / height travel, wired to interrupt pins, to auto-cut power on full extend/retract and prevent motor stall.
